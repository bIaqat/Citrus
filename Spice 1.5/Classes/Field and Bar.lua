Spice.Instance.newClass("Field",function(siz,...)
	...
	local field = Spice.Instance.newInstance("Frame",nil,{Position = ud(0,pos or 0,2),Transparency = 1,Size = not rest and ud(1,siz,4) or ud(1,0,1),BorderSizePixel = 0})
	if br then
		local bre = Spice.Instance.newInstance('Frame',field,{Size = ud(1,-(pad or 0)*2,0,1),BackgroundColor3 = brcol or hex'#e9e9e9',BorderSizePixel = 0,Position = ud(0,pad or 0,1,0),ap = v2(0,1)})
	end
	if rest then
...
Spice.Instance.newClass("Bar",function(thick,width)
	thick = thick or 1
	local frame = Spice.Instance.newObject("Frame",{Thickness = thick},{siz = ud(width or UDim.new(0,0), um(0,thick))})
	...
	function frame:TweenColor3(new, direction, style, timer,...)
		return tween(self,'BackgroundColor3',new,timer,style,direction,...)
	end
