...
RDesign = {
	Settings = {};
	Theme = {
	Pri = Color3.new(1,0,0);
	Sec = Color3.new(1,1,1);
	Theme = 'light'
	};
	...
	new = function(x,y,z)
	local main = RDesign
	local index = main.Colors[x]
	local color = index.C500
	if y then
	for i,v in pairs(index)do
		if tonumber(i:sub(2)) == y and tostring(i):sub(1,1) == 'C' or tostring(i) == 'A' then
			color = v
		end
	end
	end
	if z then
	for i,v in pairs(index)do
		if tostring(i):sub(2) == y+(z*100) and tostring(i):sub(1,1) == 'C' then
			color = v
		end
	end
	end
	...
	fromTheme = function(num,typ)
		...
		return t[num]
			...
			return main.Theme[num]
		end
	end;
	setTheme = function(index,color,t)
		local main = RDesign
		if type(color) == 'userdata' then
			color = 'Color3.new('..tostring(color)..')'
		end
		if not t then
		loadstring(index..' = '..color)()
		end
		main.Settings[index] = loadstring("return "..color)
	end;
	newTheme = function(x,y,z)
		local main =RDesign
		if x then
			main.Theme.Pri = x
		end
