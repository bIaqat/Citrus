Spice.Instance.newClass("RoundedFrame",function(round)
	...
	local Class = {
		...
		setRound = function(self,new,d)
			if self.Object.Round ~= new or d then
				...
				for i,v in next, d and d:GetChildren() or self.Rounded:GetChildren() do
					if Spice.Instance.isAnObject(v) then
						Spice.Instance.getObjectOf(v).Radius = new/2					
					elseif v:IsA('GuiObject') and v.Name ~= 'Border' and v.Name ~= 'Main' then
						local si = Spice.Misc.switch(ud(1,-new*2,0,new), ud(0,new,1,-new*2)):Filter(1,0)(v.Size.X.Scale)
						v.Size = si
		...
		setTransparency = function(self,new)
			...
			for i,v in next, self.Rounded:GetChildren() do
				if Spice.Properties.hasProperty(v,Spice.Misc.dynamicType(v)..'Transparency') and v.Name ~= 'Border' then
					v[Spice.Misc.dynamicType(v)..'Transparency'] = new
				end
			end
		end;
		setColor = function(self,new,d)
			for i,v in next, not d and self.Rounded:GetChildren() or d:GetChildren() do
				if Spice.Properties.hasProperty(v,Spice.Misc.dynamicType(v)..'Color3') then
					v[Spice.Misc.dynamicType(v)..'Color3'] = new
				end
			end			
		end;
	...
	for i = 1,4 do
		...
		local fram = Spice.Instance.newInstance("Frame",rounded,{siz = stuff[i%2 == 0 and 2 or 1], pos = stuff[2+i], ap = stuff[6+i], bsp = 0, nam = stuff[10+i], bac = Color3.new(.2,.2,.2)})
	end
	
	local border = Spice.Instance.newInstance('Frame',{siz = ud(1),trans = 1,vis = false,nam = "Border",par = rounded, ap = v2(.5), pos = ud(.5), siz = ud(1)})
	for i,v in next,rounded:GetChildren() do
		if v.Name ~= 'Border' then
			local b = (Spice.Instance.getObjectOf(v) or v):Clone()
			local c = (Spice.Instance.getObjectOf(v) or v):Clone()
			v:Destroy()
			c.Parent = rounded
			b.Parent = border
		end
	...
	Spice.Instance.newInstance("Frame",rounded,{nam = "Main", ap = v2(.5), siz = ud(1,-round*2,3), pos = ud(.5),bsp = 0; bac = Color3.new(.2,.2,.2)})
	
	local new = Spice.Instance.newObject("Frame",Class,{trans = 1})
	new.RoundValue = Spice.Instance.newInstance("IntValue",rounded,{nam = 'Round'})
	...
	new.TweenRound = function(self,round,direction,style,timer)
		Spice.tweenService(self.RoundValue, 'Value', round, timer, style, direction)
	end
