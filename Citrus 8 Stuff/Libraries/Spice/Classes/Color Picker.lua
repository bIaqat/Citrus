Citrus.object:newClass('ColorPicker',function(typ)
	local create = Citrus.object:newVanillaObject
	local colorMixer = Citrus.object:newCustomObject('Frame',nil,{bt = 1},
		{
			Bar = create('Frame',nil,{pos = ud(1,3,0,0),siz = ud(.15,1,1)});
			BarCursor = create('Frame',nil,{bc3 = Color3.new(1,1,1),bsp = 2,siz = ud(1,3,0,5),pos = ud(.5,0,1),ap = v2(.5)});
			Mixer = create('Frame',nil,{siz = ud(1), bt = 1});
			MixerCursor = create('Frame',nil,{siz = ud(25,2), bt = 1,ap = v2(.5)});
	...
	set(create('ImageLabel',colorMixer.Mixer,{siz = ud(1), bt = 1}):Clone(),{
	...
	set(create('Frame',colorMixer.MixerCursor,{bac = Color3.new(1,1,1),ap = v2(.5,0), pos = ud(.5,0,1), siz = ud(.1,1,1), bsp = 0}):Clone(),{
	...
	function setColor(basedon,a,b)
		...
		colorMixer.Properties.Color = col
		colorMixer.Properties.colorInHex = Citrus.color.toHex(col)
