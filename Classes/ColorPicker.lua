Spice.Instance.newClass('ColorPicker',function(typ)
	local colorMixer = Spice.Instance.newObject('Frame',nil,
		{
			Bar = Spice.Instance.newInstance('Frame',{pos = ud(1,3,0,0),siz = ud(.15,1,1)});
			BarCursor = Spice.Instance.newInstance('Frame',{bc3 = Color3.new(1,1,1),bsp = 2,siz = ud(1,3,0,5),pos = ud(.5,0,1),ap = v2(.5)});
			Mixer = Spice.Instance.newInstance('Frame',{siz = ud(1), bt = 1});
			MixerCursor = Spice.Instance.newInstance('Frame',{siz = ud(25,2), bt = 1,ap = v2(.5)});
	...
	set(Spice.Instance.newInstance('ImageLabel',colorMixer.Mixer,{siz = ud(1), bt = 1}):Clone(),{
	...
	set(Spice.Instance.newInstance('Frame',colorMixer.MixerCursor,{bac = Color3.new(1,1,1),ap = v2(.5,0), pos = ud(.5,0,1), siz = ud(.1,1,1), bsp = 0}):Clone(),{
