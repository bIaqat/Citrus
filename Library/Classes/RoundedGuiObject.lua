Spice.Objects.Classes.new("RoundedGuiObject",function(radius) --radius is udim2
	...
	Spice.Objects.newInstance('Frame',roundFolder,{Name = 'Center',BorderSizePixel = 0;BackgroundColor3 = Color3.new(.2,.2,.2)})
	for i = 1,4 do
		local roundSize = radii[i]
		local c = Spice.Objects.newInstance('ImageLabel',roundFolder,{
		...
		Spice.Objects.newInstance('IntValue', radiusFolder, {Name = c.Name, Value = roundSize}).Changed:connect(function(a)
	...
	for i = 1,4 do
		local roundSize = radii[i]
		Spice.Objects.newInstance("Frame",roundFolder,{
